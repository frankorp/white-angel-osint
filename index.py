...
║  ╚═════╝ ╚══════╝╚══════╝╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚══════╝║
║                                                              ║
║                А Н Г Е Л   О С Л Е Д О В А Н И Й            ║
║                     Версия 2.0 • Белый Код                  ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
        """
    ...
    def анализ_ip(self, ip):
        ...
        прокси_чек = self.запрос_безопасный(f"https://vpnapi.io/api/{ip}")
        ...
        for порт in порты[:5]:  # Только первые 5 для скорости
            try:
                ...
                сокет.settimeout(1)
                результат = сокет.connect_ex((ip, порт))
                if результат == 0:
    ...
    def анализ_домена(self, домен):
        ...
        try:
            ...
            with socket.create_connection((домен, 443), timeout=5) as sock:
                with контекст.wrap_socket(sock, server_hostname=домен) as ssock:
                    ...
                    результаты.append(f"   Издатель: {сертификат['issuer']}")
        except:
            результаты.append("🔐 \033[91mSSL не обнаружен\033[0m")
        ...
        if 'заголовки' in заголовки:
            ...
            for ключ, значение in заголовки['заголовки'].items():
                if любой(к.lower() in ключ.lower() for к in важные):
    ...
    def анализ_телефона(self, телефон):
        ...
        for префикс, оператор in операторы.items():
            if чистый.replace('+', '').startswith(префикс):
        ...
        соцсети = {
            'Telegram': f'https://t.me/{чистый.replace("+", "")}',
            'WhatsApp': f'https://wa.me/{чистый.replace("+", "")}',
            'Viber': f'viber://chat?number={чистый}',
            'VK': f'https://vk.com/phone/{чистый}'
        }
    ...
    def анализ_почты(self, почта):
        ...
        try:
            mx_записи = socket.getaddrinfo(f'mx.{домен}', 25)
            результаты.append(f"📨 \033[92mMX ЗАПИСИ:\033[0m Найдены")
        except:
            результаты.append(f"📨 \033[91mMX ЗАПИСИ:\033[0m Не найдены")
    ...
    def поиск_никнейма(self, никнейм):
        ...
        платформы = [
            ...
            ('LinkedIn', f'https://linkedin.com/in/{никнейм}'),
            ('Spotify', f'https://open.spotify.com/user/{никнейм}'),
            ('SoundCloud', f'https://soundcloud.com/{никнейм}'),
            ('DeviantArt', f'https://deviantart.com/{никнейм}'),
            ('Flickr', f'https://flickr.com/people/{никнейм}')
        ]
...
def любой(итератор):
    """Замена для built-in any() если не доступно"""
